...
public class MusicSlider {
    ...
    public MusicSlider(int x, int y, int width, int height) {
        ...
        slider.setVisualPercent(Float.parseFloat(configReader.getGeneralProperties("Music_Volume")));
        this.volume = Float.parseFloat(configReader.getGeneralProperties("Music_Volume"));
        slider.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {
                volume = (slider.getValue() / 100f) * 100;
                configReader.updateGeneralProperties("Music_Volume", volume + "");
            }
        });
    }
